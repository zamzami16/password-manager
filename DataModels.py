...
class dataModel:
    ...
    def initData(self):
        """
        check password users and if not exists, create it
        """
        try:
            print('debug piye yo')
            users = crp.read_encrypted(path=self.users_file,
            ...
            crp.to_encrypted(df, password=self.mainPassword,
    ...
    def initData1(self):
        ...
        """
        try:
            users = crp.read_encrypted(path=self.listPasswordFile,
                                       password=self.mainPassword,
                                       salt=self.salt)
            return users
        except FileNotFoundError:
            ...
            crp.to_encrypted(df, password=self.mainPassword,
                             ...
                             salt=self.salt)
            user = crp.read_encrypted(path=self.listPasswordFile,
                                      password=self.mainPassword,
                                      salt=self.salt)
            return user
    ...
    def add_data_user(self, user, pwd):
        ...
        self.users = self.users.append({'users': user,
                                        'passwords': pwd}, ignore_index=True)
    ...
    def commitData(self, data='users'):
        ...
        if data == 'users':
            crp.to_encrypted(self.users, password=self.mainPassword,
                             path=self.users_file, salt=self.salt)
        elif data == 'site':
            crp.to_encrypted(self.listPassword, password=self.mainPassword,
                             path=self.listPasswordFile, salt=self.salt)
    ...
    def change_password_user(self, user, newPwd):
        ...
        self.users.loc[
            self.users.users == user, 'passwords'
        ] = newPwd
        self.commitData(data='users')
